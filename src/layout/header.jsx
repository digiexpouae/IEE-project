    ...
    <header className={`fixed w-full z-50 transition-all flex flex-col items-center justify-center duration-300 ${isScrolled ? 'bg-[#A81F19] shadow-md' : 'bg-[#A81F19]'}`}>
      ...
      <div className="md:container relative lg:absolute lg:-bottom-10 left-0 right-0 mx-auto md:px-4 py-3 flex justify-between xl:justify-center items-center w-full lg:w-[80%] xl:w-[70%]  bg-[#A81F19] md:bg-transparent">
       ...
       <div className="md:flex items-center space-x-6 py-2 px-4  text-xs xl:text-sm  whitespace-nowrap">  <Link href="/" className="text-white hover:text-gray-200 font-medium transition">Home</Link>
          ...
          <Link href="/projects" className="text-white hover:text-gray-200 font-medium transition">Donate</Link>
      ...
      {/* Mobile Menu - Sidebar */}
      <div 
        className={`fixed top-0 right-0 h-full w-4/5 max-w-sm bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50 lg:hidden ${
          isMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="h-full overflow-y-auto">
          {/* Mobile Menu Header */}
          <div className="bg-[#A81F19] p-4 flex justify-between items-center">
